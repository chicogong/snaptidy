...
| `detect_bad_extensions.py` | "bad extension", "扩展名校验" | Detect files whose magic bytes don't match their extension; 20+ format signatures; `--parallel`, `--incremental`, `--report` |
...
| Multi-dim quality | (enhanced) | `assess_quality.py` now computes 7 dimensions: sharpness, exposure, contrast, resolution, format, filesize, EXIF completeness |
...
| Script | Trigger | What it does |
|--------|---------|-------------|
| `assess_quality.py` | "照片质量", "quality assessment" | Blur/brightness/contrast → Q0-100 score, auto-used in dedup strategy & review page; 7-dimension scoring (v3.13+) |
